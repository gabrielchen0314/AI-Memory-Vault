...
ai_summary: "程式專案在 Vault 建立知識庫的同時，必須在專案原始碼資料夾建立 API Map，保持雙向同步"
tags: [rule, workflow, documentation, api-map]
---

# 03 — 專案內 API Map 同步規則

> 程式相關專案在 AI 資料庫建立知識結構的同時，**必須在專案原始碼目錄本身**也建立 API Map，加速後續開發作業。
...
1. **Vault 側**：在 `work/{COMPANY}/projects/{PROJECT}/` 建立標準知識庫文件（依 `templates/projects/{type}/TEMPLATE.md`）
